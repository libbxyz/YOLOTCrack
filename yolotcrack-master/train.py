...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'tripl', 'labels', 'img_size'))
        if RANK in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                pred, features = model(imgs)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device), features)  # loss scaled by batch_size
                if RANK != -1:
